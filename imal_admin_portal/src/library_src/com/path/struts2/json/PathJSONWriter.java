...
class PathJSONWriter {
    ...
    private void process(Object object, Method method) throws JSONException {
        ...
        {
        	Object theObj = object;
        	//PathSolutions: for large bigdecimal values they were getting truncated after decimal point
        	if(theObj instanceof BigDecimal)
        	{
        	    	//944888 BB191178 check if the toString value has exponential representation, to make the value as string quoted
        	    	boolean isExponential = theObj.toString().toLowerCase().contains("e");
        		theObj = ((BigDecimal)theObj).toPlainString();
        		
        		// TP #879429 : send bigDecimal as string to JS to solve limitation in the Number format of JS 
        		if(theObj.toString().split("\\.")[0].length() > 15 || isExponential) 
        		{
        			this.string(theObj);
        		}
        		else 
        ...
        try {
            ...
            for (int i = 0; i < props.length; ++i) {
                ...
                if (baseAccessor != null) {
                    ...
                    //PathSolutions: removing empty lists,arrays and maps
            ...
            if(!object.getClass().getName().contains("EnumModels"))
            {
    ...
    private boolean shouldExcludeProperty(PropertyDescriptor prop) throws SecurityException,
        ...
        return ("class".equals(name) || "declaringClass".equals(name) || "cachedSuperClass".equals(name)
    ...
    private void map(Map map, Method method) throws JSONException {
        ...
        while (it.hasNext()) {
            ...
            if (!warnedNonString && (null == key || !(key instanceof String))) {
                LOG.warning("JavaScript doesn't support non-String keys, using toString() on "
    ...
    private void date(Date date, Method method) {
        ...
        {
            this.formatter = new SimpleDateFormat(PathJSONUtil.RFC3339_FORMAT, Locale.ENGLISH);
        }

        DateFormat formatter = (json != null) && (json.format().length() > 0) ? new SimpleDateFormat(json
                .format(),Locale.ENGLISH) : this.formatter;
