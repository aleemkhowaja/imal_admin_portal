/*
 * $Id: JSONResult.java 799110 2009-07-29 22:44:26Z musachy $
 *
 ...
 * </pre>
 */
public class PathJSONResult implements Result {
    private static final long serialVersionUID = 8624350183189931165L;
    private static final Logger LOG = LoggerFactory.getLogger(PathJSONResult.class);
    ...
    private boolean enumAsBean = PathJSONWriter.ENUM_AS_BEAN_DEFAULT;
    ...
    public void setExcludeProperties(String commaDelim) {
        List<String> excludePatterns = PathJSONUtil.asList(commaDelim);
    ...
    public void setIncludeProperties(String commaDelim) {
        List<String> includePatterns = PathJSONUtil.asList(commaDelim);
            ...
            for (String pattern : includePatterns) {
                ...
                for (String patternPiece : patternPieces) {
                    if (patternExpr.length() > 0) {
                        patternExpr = patternExpr.concat("\\.");
                    }
                    patternExpr = patternExpr.concat(patternPiece);
    ...
    public void execute(ActionInvocation invocation) throws Exception {
        ...
        try {
            ...
            json = PathJSONUtil.serialize(rootObject, excludeProperties, includeProperties, ignoreHierarchy,
                    enumAsBean, excludeNullProperties);
            json = addCallbackIfApplicable(request, json);

            boolean writeGzip = enableGZIP && PathJSONUtil.isGzipInRequest(request);
    ...
    protected void writeToResponse(HttpServletResponse response, String json, boolean gzip)
            throws IOException {
        PathJSONUtil.writeJSONToResponse(new SerializationParams(response, getEncoding(), isWrapWithComments(),
    ...
    protected org.apache.struts2.json.smd.SMD writeSMD(ActionInvocation invocation) {
        ...
        // get public methods
        Method[] methods = PathJSONUtil.listSMDMethods(clazz, ignoreInterfaces);
            ...
            else 
            {
                ...
                if (parametersCount > 0) {
                    ...
                    for (int i = 0; i < parametersCount; i++) {
                        ...
                        String paramName = smdMethodParameterAnnotation == null?"p".concat(String.valueOf(i))
                                :smdMethodParameterAnnotation.name();
    ...
    protected String addCallbackIfApplicable(HttpServletRequest request, String theJson) {
	String json = theJson;
        if ((callbackParameter != null) && (callbackParameter.length() > 0)) {
            String callbackName = request.getParameter(callbackParameter);
            if ((callbackName != null) && (callbackName.length() > 0))
            {
                json = callbackName.concat("(").concat(json).concat(")");
            }
        }
        return json;
    }
